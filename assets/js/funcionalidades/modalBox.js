...
import { obtenerCorreos } from "./contextMenu/acciones/compartir.js";
...
export async function mostrarModalInfoCarpeta(idRuta, tipo) {
    ...
    const infoNombreCarpeta = document.querySelector(".info_nombre_carpeta"),
        nombreCarpeta = document.querySelector(".info_nombre_completo_carpeta"),
    ...
    nombreCarpeta.textContent = `"${datosCarpeta.nombre_carpeta}"`;
...
export async function mostrarModalInfoArchivo(idRuta, tipo) {
    ...
    const infoNombreArchivo = document.querySelector(".info_nombre_archivo"),
        nombreArchivo = document.querySelector(".info_nombre_completo_archivo"),
    ...
    nombreArchivo.textContent = `"${datosArchivo.nombre_archivo}"`;
...
/**
 * Función asíncrona que obtiene el nombre del elemento (archivo o carpeta) para mostrar en el modal de mover
 * @param {string} idRuta - El ID de la ruta del elemento
 * @param {string} tipo - El tipo de elemento (carpeta o archivo)
 */
export async function obtenerNombreElemento(idRuta, tipo) {
    try {
        let url;
        let formData = new FormData();
        formData.append("idRuta", idRuta);

        // URL del controlador según su tipo de elemento
        if (tipo === "carpeta") {
            url = `mvc/controllers/modalBox/modal_nombre_carpeta.php`;
        } else {
            url = `mvc/controllers/modalBox/modal_nombre_archivo.php`;
        }

        // Se realiza la solicitud
        const data = await fetchDataBody(url, formData);

        // Mostrar el nombre del elemento en el modal de mover
        const nombreElemento = document.querySelector(".mover_nombre_elemento");
        nombreElemento.textContent = `"${data}"`;
    } catch (error) {
        // Manejar cualquier error que ocurra durante el proceso
        console.error(error);
    }
...
let inputCompartir = document.querySelector(".input_compartir");

// Obtener correos disponibles para compartir
const respuestaCorreo = await obtenerCorreos();

let correos;
if (respuestaCorreo.respuesta === "Exitoso") {
    correos = respuestaCorreo.correos;
}
correos = correos.sort();

/**
 * Función que muestra los correos disponibles para compartir según la entrada del usuario
 * @param {string} valor - El valor introducido por el usuario
 */
function mostrarCorreos(valor) {
    inputCompartir.value = valor;
    removerElementos();
}

/**
 * Función que remueve los elementos de la lista de correos
 */
function removerElementos() {
    let items = document.querySelectorAll(".lista-email");
    items.forEach((item) => {
        item.remove();
    });
}

// Evento de teclado para filtrar y mostrar correos al escribir
inputCompartir.addEventListener("keyup", (e) => {
    removerElementos();
    // Iterar por cada correo
    for (let i of correos) {
        // Si coincide con lo que ingresa el usuario
        if (i.toLowerCase().startsWith(inputCompartir.value.toLowerCase()) && inputCompartir.value != "") {
            // Se crea un elemento de lista y se muestra al usuario
            let listItem = document.createElement("li");
            listItem.classList.add("lista-email");
            listItem.textContent = i;
            document.querySelector(".lista-emails").appendChild(listItem);

            // Si se hace clic en el correo, el input es asignado con el correo elegido
